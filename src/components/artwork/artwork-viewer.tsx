...
import LayerControlLink from './layer-control-link';
...
export default function ArtworkViewer({
  ...
  const { artElementRef, statusMessage, metadata, collector, error, layers } =
    ...
    <>
      <div
        className={`flex items-center justify-center ${
          isFullscreen
            ? 'fixed top-0 left-0 w-screen h-screen bg-black z-50'
            : artContainerClassName || ''
        }`}
      >
        {statusMessage && (
      ...
      {isLayersModalOpen && (
        <Modal title="Layers" onClose={() => setIsLayersModalOpen(false)}>
          <ul>
            {layers?.map((layer: MasterArtNFTMetadata['layout']['layers'][0]) => (
              <li key={layer.id}>
                <LayerControlLink layer={layer} />
                {layer.uri && (
                  <a
                    href={getIpfsUrl(layer.uri)}
                    target="_blank"
                ...
                {layer.activeStateURI && (
                  <a
                    href={getIpfsUrl(layer.activeStateURI)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="ml-2 text-blue-500"
                  >
